...
describe('setup test', () => {
    it('should have working action creators', () => {
        expect(setups.updatePower('eclipse', 5000, 1).payload).toStrictEqual({ type: 'eclipse', value: 5000, level: 1 })
        expect(setups.updatePower('regular', 4000, 9).payload).toStrictEqual({ type: 'regular', value: 4000, level: 9 })

        expect(setups.updateLuck('eclipse', 30, 1).payload).toStrictEqual({ type: 'eclipse', value: 30, level: 1 })
        expect(setups.updateLuck('regular', 25, 9).payload).toStrictEqual({ type: 'regular', value: 25, level: 9 })

        expect(setups.updateChampFire('eclipse', true, 1).payload).toStrictEqual({
            type: 'eclipse',
            value: true,
            level: 1
        })
        expect(setups.updateChampFire('regular', false, 17).payload).toStrictEqual({
            type: 'regular',
            value: false,
            level: 17
        })

        expect(setups.updateSpeed('eclipse', 9, 1).payload).toStrictEqual({ type: 'eclipse', value: 9, level: 1 })
        expect(setups.updateSpeed('regular', 5, 9).payload).toStrictEqual({ type: 'regular', value: 5, level: 9 })

        expect(setups.updateSiphon(20, 1).payload).toStrictEqual({ value: 20, level: 1 })
    })
